...
export default function IngestionStatus() {
  ...
  const fetchJobs = async () => {
    try {
      ...
      // const response = await fetchWithAuth<IngestionJob[]>('/ingestion/jobs');
  ...
  const handleCancelJob = async (jobId: string) => {
    try {
      ...
      setIngestionJobs(ingestionJobs.map(job => 
        job.id === jobId ? { ...job, status: 'failed', error: 'Job cancelled by user' } : job
      ));
    } catch (error) {
  ...
  const handleRetryJob = async (jobId: string) => {
    try {
      ...
      setIngestionJobs(ingestionJobs.map(job => 
        job.id === jobId ? { ...job, status: 'processing', progress: 0, error: undefined } : job
      ));
    } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: (job: IngestionJob) => {
        const statusVariant = 
          ...
          job.status === 'processing' || job.status === 'queued' ? 'warning' : 
          'error';
    ...
    {
      header: 'Completed',
      accessor: (job: IngestionJob) => job.completedAt 
